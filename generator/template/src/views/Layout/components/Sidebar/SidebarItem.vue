...
div
  ...
  el-menu-item(v-if="item.meta && item.meta.href", :index="getHref(item.meta, item)")
    i.fa(v-if="item.meta.icon", :class="item.meta.icon", aria-hidden="true")
    span(v-if="item.meta.title", slot="title") {{ generateTitle(item.meta.title) }}
</template>
<script lang="ts">
...
export default class SidebarItem extends Mixins(RouterTitleMixin) {
  ...
  getHref(meta: any = {}, route: any) {
    return meta.href ? `isTurnByHref__${meta.href}` : meta.title || route.name || route.path;
  }
}
</script>
