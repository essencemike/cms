...
import { API_DEFAULT_CONFIG, BASE_API } from '@/config';
...
class ApiMaker {
  ...
  _apiSingleBuilder({
    ...
    config.forEach((api) => {
      const { name, desc, method, path, mockPath, query } = api;
      const originUrl = mock ? mockPath : path;
      const baseURL = mock ? mockBaseURL : BASE_API;
      ...
      this.api[namespace][name] = new Proxy(() => {}, {
        apply: (target, thisArg, argumentsList) => {
          ...
          const data = isEmpty(outerParams) ? {} : { ...outerParams };
          return axios(_normalize(Object.assign({}, {
...
function _normalize(options: any, data: any) {
